    ...
    <div className="orchestrator-v2">
      ...
      <div className="command-center-header">
        ...
        <div className="header-actions">
          ...
          >
            <BarChart3 size={16} />
            {drawerExpanded ? 'Hide Details' : 'Show Details'}
          </button>
        </div>
      </div>
      ...
      <div className="command-center-grid">
        ...
        <div className="score-zone panel">
          ...
          <div className="kpi-pills">
            <div className="kpi-pill foundation">
              <span className="kpi-label">Foundation</span>
              <span className="kpi-value">{overallProgress.foundational.percentage}%</span>
            </div>
            <div className="kpi-pill critical">
              <span className="kpi-label">Critical</span>
              <span className="kpi-value">{overallProgress.foundational.criticalCompleted}/{overallProgress.foundational.criticalTotal}</span>
            </div>
            <div className="kpi-pill ongoing">
              <span className="kpi-label">Ongoing</span>
              <span className="kpi-value">{overallProgress.ongoing.percentage}%</span>
            </div>
          </div>
        ...
        <div className="action-zone panel">
        ...
        <div className="progress-zone panel">
      ...
      {/* Compact Checklist Drawer */}
      {drawerExpanded && (
        <div className="checklist-drawer">
          <div className="drawer-header">
            <div className="drawer-tabs">
              <button 
                className={`drawer-tab ${activeTab === 'foundation' ? 'active' : ''}`}
                onClick={() => setActiveTab('foundation')}
              >
                Foundation ({foundationalCategories.length})
              </button>
              <button 
                className={`drawer-tab ${activeTab === 'ongoing' ? 'active' : ''}`}
                onClick={() => setActiveTab('ongoing')}
              >
                Ongoing ({ongoingCategories.length})
              </button>
              <button 
                className={`drawer-tab ${activeTab === 'completed' ? 'active' : ''}`}
            ...
            {activeTab === 'foundation' && (
              <div className="compact-checklist">
                {foundationalCategories.map(category => (
                  <div key={category.category_id} className="compact-category">
                    <h5 className="compact-category-title">{category.icon} {category.name}</h5>
                    <div className="compact-items">
                      {category.items.map(item => (
                        <div key={item.item_id} className="compact-item">
                          <input
                            type="checkbox"
                            checked={item.status === 'completed'}
                            onChange={() => handleCheckboxChange(item.item_id, item.status)}
                            disabled={updatingItems[item.item_id]}
                          />
                          <span className={`compact-item-title ${item.status === 'completed' ? 'completed' : ''}`}>
                            {item.title}
                            {item.is_critical === 1 && <Zap size={12} className="critical-badge" />}
                          </span>
                        </div>
                      ))}
                    </div>
                    ...
                    <div key={category.category_id} className="compact-category">
                      ...
                      <div className="compact-items">
                        {completedItems.map(item => (
                          <div key={item.item_id} className="compact-item completed">
                            <CheckCircle size={16} />
                            <span className="compact-item-title completed">{item.title}</span>
                          </div>
                        ))}
                      </div>
                    </div>
